class PeopleController < ApplicationController
  ...
  def export_to_csv
    csv_string = CSV.generate do |csv|
      ...
      unless admin
        Organization.find(current_client.organization.id).uploads.includes(:person).find_in_batches(batch_size: 5000) do |group|
      ...
      if admin && params[:organization_id]
        Organization.find(params[:organization_id]).uploads.includes(:person).find_in_batches(batch_size: 5000) do |group|
          group.each do |upload|
            if upload.is_device_input == 1
              input_method = "Synced from Device"
            else
        ...
        Organization.all.uploads.includes(:person).find_in_batches(batch_size: 5000) do |group|
          group.each do |upload|
            if upload.is_device_input == 1
              input_method = "Synced from Device"
            else
